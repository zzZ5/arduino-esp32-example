...
bool doMeasurementAndSave() {
  ...
  if (msgDetail.length() > 300) msgDetail = msgDetail.substring(0, 300);
  doc["info"]["msg"] = msgDetail;
  doc["info"]["heat"] = heaterIsOn;
  doc["info"]["pump"] = pumpIsOn;
  doc["info"]["aeration"] = aerationIsOn;
...
void setup() {
  ...
  StaticJsonDocument<1024> bootDoc;
  bootDoc["device"] = appConfig.equipmentKey;
  bootDoc["status"] = "online";
  bootDoc["timestamp"] = nowStr;
  bootDoc["last_measure_time"] = lastMeasStr;
  bootDoc["mode"] = currentMode;

  // ---- 控制配置信息 config ----
  JsonObject cfg = bootDoc.createNestedObject("config");

  // 基础节拍
  cfg["post_interval"] = appConfig.postInterval;

  // 温度限值
  JsonObject limits = cfg.createNestedObject("temp_limits");
  ...
  // 水箱安全
  JsonObject safety = cfg.createNestedObject("safety");
  safety["tank_temp_max"] = appConfig.tankTempMax;

  // 加热器防抖
  JsonObject hg = cfg.createNestedObject("heater_guard");
  hg["min_on_ms"] = appConfig.heaterMinOnMs;
  hg["min_off_ms"] = appConfig.heaterMinOffMs;

  // 泵自适应阈值
  JsonObject pa = cfg.createNestedObject("pump_adaptive");
  ...
  // 泵学习参数
  JsonObject pl = cfg.createNestedObject("pump_learning");
  ...
  // n-curve diff 曲线参数
  JsonObject curves = cfg.createNestedObject("curves");
  curves["in_diff_ncurve_gamma"] = appConfig.inDiffNCurveGamma;

  // Setpoint 模式参数
  JsonObject bs = cfg.createNestedObject("bath_setpoint");
  bs["enabled"] = appConfig.bathSetEnabled;
  bs["target"] = appConfig.bathSetTarget;
  bs["hyst"] = appConfig.bathSetHyst;

  // 曝气定时
  JsonObject aer = cfg.createNestedObject("aeration_timer");
  aer["enabled"] = appConfig.aerationTimerEnabled;
  aer["interval"] = appConfig.aerationInterval;
  aer["duration"] = appConfig.aerationDuration;

  // 传感器 key（方便后端检查是否配置错）
  JsonObject keys = cfg.createNestedObject("keys");
  keys["temp_in"] = appConfig.keyTempIn;
  JsonArray kout = keys.createNestedArray("temp_out");
  for (size_t i = 0; i < appConfig.keyTempOut.size(); ++i) {
    kout.add(appConfig.keyTempOut[i]);
  }
