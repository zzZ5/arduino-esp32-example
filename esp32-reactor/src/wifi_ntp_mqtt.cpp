...
bool connectToWiFi(unsigned long timeoutMs) {
	WiFi.mode(WIFI_STA);
	WiFi.begin(appConfig.wifi.ssid.c_str(), appConfig.wifi.password.c_str());  // 修改为appConfig.wifi

	Serial.print("[WiFi] Connecting to: ");
	Serial.println(appConfig.wifi.ssid);
...
bool multiNTPSetup(unsigned long totalTimeoutMs) {
	...
	while (!synced) {
		for (const auto& server : appConfig.ntpServers) {  // 修改为appConfig.ntpServers
	...
	// 设置时区：东八区
	configTime(8 * 3600, 0, appConfig.ntpServers[0].c_str());  // 修改为appConfig.ntpServers
...
bool connectToMQTT(unsigned long timeoutMs) {
	mqttClient.setServer(appConfig.mqtt.server.c_str(), appConfig.mqtt.port);  // 修改为appConfig.mqtt.server 和 appConfig.mqtt.port
	...
	while (!mqttClient.connected()) {
		...
		Serial.printf("[MQTT] Connecting to %s:%d...\n", appConfig.mqtt.server.c_str(), appConfig.mqtt.port);  // 修改为appConfig.mqtt.server
		if (mqttClient.connect(appConfig.mqtt.clientId.c_str(),  // 修改为appConfig.mqtt.clientId
			appConfig.mqtt.user.c_str(),
			appConfig.mqtt.pass.c_str())) {
			Serial.println("[MQTT] Connected.");

			// 连接成功后重新订阅响应 topic
			if (appConfig.mqtt.responseTopic.length() > 0) {
				if (mqttClient.subscribe(appConfig.mqtt.responseTopic.c_str())) {  // 修改为appConfig.mqtt.responseTopic
